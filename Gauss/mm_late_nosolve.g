@  Estiamtion of tau_LATE as in SUW (2024) by weighting methods
...
Version 1.0
        ...
        - mm_late_nosolve: procedure to get the coeffecient estimates and se. uses as input the moment function f1
        and  f2 ( the OPG estimator -outer procut of moments)
        return:
 f1: can be any of the moment functions (normlate_mom, normkappalet_mom, kappalte_mom. kappa1late_mom, kappa0late_mom
 f2: OPG function correponding to f1 functions are defined here.
output:

ds structure is the third input and involves 
...
dlate[6].type = n; 
Output:
   outlate.late  = latehat; late estimate
   outlate.latese  = se_late; standard error
   outlate.pars=  estcoef; all coeffcient estimates including coefficients of IPS model
   outlate.parsse = sqrt(diag(v_mest)); all standard errors
...
proc(1) =  mm_late_nosolve(&f1, &f2,  struct DS ds_struct); //&f3,
    library dc;
...
// Step four: Call binary logit procedure
    ...
    elseif pf == 3;//CBPS
        ...
        struct OutputsCBPS est_cbps;
    est_cbps = cbps(&cbps_mom,&cbps_crossmom, ds_struct);    
        alp_start=(est_cbps.pars);
    endif;
...
*/
    latehat=taus;
    estcoef=pvGetParVector(par);
        v_mest=(1/N)*inv(gradMT(&f1,par, ds_struct))*f2(par, ds_struct)*inv(gradMT(&f1,par, ds_struct))';
...
proc kappa1late_crossmom(struct PV p, struct DS ds_struct);
...
proc kappa0late_crossmom(struct PV p, struct DS ds_struct);
local psi1, alp, gammah, deltah, tauh, psi2, psi3, psi4;
     alp = pvUnpack(p, "alphahat");
     gammah = pvUnpack(p, "gammahat");
     deltah = pvUnpack(p, "deltahat");
...
proc kappalate_crossmom(struct PV p, struct DS ds_struct);
local psi1, alp, gammah, deltah, tauh, psi2, psi3, psi4;
     alp = pvUnpack(p, "alphahat");
     gammah = pvUnpack(p, "gammahat");
     deltah = pvUnpack(p, "deltahat");
...
proc normkappalate_crossmom(struct PV p, struct DS ds_struct);
...
proc normlate_crossmom(struct PV p, struct DS ds_struct);
